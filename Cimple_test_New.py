...
word, token, variables, ins, inouts, temp, elseflag, whileflag = "","", list(), list(), list(), "", 0, 0
...
katastaseis = ["declarations","","statements"]
#----------------------------------
...
2. functions (if any) and 3. statements (if any). Error at line: "+str(int((file_counters[1] + 1) / 2))+",column: " +(str(file_counters[4] - len(word))))
                        file.close()
                        sys.exit()
...
def statements(file_counters):
    global word, token, temp, elseflag, whileflag
    counter_statements = 0
    forreturn = ""
    tempreturn = ""
    ...
    if(token == "begintk"):
        ...
        tempreturn = statement(file_counters)
        forreturn = forreturn +" "+ tempreturn
        if(tempreturn != ""):
            counter_statements += 1
        ...
        if(token == "endtk"):
            ...
            elif(counter_statements >= 2):
                print("Syntax Error. Keyword ';' excpected after '}' since there are two or more statements. Error at line: "+str(int((file_counters[1] + 1) / 2))+
            ",column: " +(str(file_counters[4] - len(word) - 1)))
                file.close()
                sys.exit()
        else:
        ...
        if(((word == ';') and (elseflag > 0)) or ((word == ';') and(whileflag > 0))):
            pass
        else:
            word, token = lexer(file_counters)
            print(word+" "+token)
...
2. functions (if any) and 3. statements (if any). Error at line: "+str(int((file_counters[1] + 1) / 2))+
            ...
            file.close()
            sys.exit()
        elif((temp == "subprograms") and (token == "declaretk")):
            print("Syntax Error. Wrong program structure! The valid structure for the program is 1.declarations (if any)\n\
2. functions (if any) and 3. statements (if any). Error at line: "+str(int((file_counters[1] + 1) / 2))+
            ",column: " +(str(file_counters[4] - len(word))))
            file.close()
...
def statement(file_counters):
    ...
    if(token == "keywordtk"):
        ...
        print("'Strict words' like '"+word+"' are not acceptable at this point of code.Error at line: "+str(int((file_counters[1] + 1) / 2))+
        ",column: " +(str(file_counters[4] - len(word))))
        file.close()
        sys.exit()
